...
export class ClamAVService {
  ...
  private _processResult(
    ...
    if (typeof result !== 'string') {
      this.logger?.error(
        `${this.debugLabel}: Invalid stdout from scanner (not a string): `,
        result
      );
    ...
    if (/^(.+)ERROR(\u0000|[\r\n])?/gm.test(result)) {
      ...
      this.logger?.error(`${this.debugLabel}: Error Response: `, error);
  ...
  private _initSocket(): Promise<net.Socket> {
    return new Promise((resolve, reject) => {
      ...
      client
        ...
        .on('timeout', () => {
          this.logger?.error(
            `${this.debugLabel}: Socket/Host connection timed out.`
          );
        ...
        .on('error', (e) => {
          this.logger?.error(
            `${this.debugLabel}: Socket/Host connection failed:`,
            e
          );
          reject(e);
        });
  ...
  public scanStream(stream: Readable, file?: string): Promise<ProcessResult> {
    return new Promise<ProcessResult>(async (resolve, reject) => {
      ...
      let transform: ClamTransform | undefined;
      ...
      // Helper function to safely reject once
      ...
      try {
        ...
        transform
          ...
          .on('error', (err: Error) => {
            this.logger?.error(
              `${this.debugLabel}: Transform stream error: `,
              err
            );
            safeReject(err);
          });
        ...
        stream
          ...
          .on('error', (err: Error) => {
            this.logger?.error(
              `${this.debugLabel}: Error from input stream (perhaps the uploader closed the browser?)`,
              err
            );
            safeReject(err);
          });
        ...
        socket
          ...
          .on('close', (hadError: boolean) => {
            if (hadError) {
              this.logger?.error(
                `${this.debugLabel}: ClamAV socket closed due to an error.`
              );
            }
          })
          .on('error', (err: Error) => {
          ...
          .on('end', () => {
            ...
            if (!finished) {
              return reject(
                new Error(
                  `Scan aborted. Reply from server: ${response.toString(
                    'utf8'
                  )}`
                )
              );
